...
│   │       └── ...
│   └── vite.config.ts
├── pkg/nfpm/               # Linux .deb/.rpm packaging (nFPM)
...
│   └── setup/
│       ├── install.sh          # Public one-liner installer (FR/EN)
...
| GET | `/api/radio/proxy` | Virtual companion TCP listen settings and live status |
| PATCH | `/api/radio/proxy` | Enable/disable/rebind the radio proxy (drops all sessions) |
...
| GET | `/api/read-state/unreads` | Server-computed unread counts, mentions, last message times/previews, `last_read_ats` boundaries, and `first_unread_ids` (unread-divider anchor) |
...
- Aggregated counts via `GET /api/read-state/unreads` (server-side computation of counts, mention flags, `last_message_times`, `last_message_previews`, `last_read_ats`, and `first_unread_ids`)
